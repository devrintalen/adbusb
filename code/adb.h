...
Apple Desktop Bus (abbreviated ADB) is a host-controlled synchronous protocol developed for the peripherals for the original Apple computer. You'll find it on old Apple peripherals, including the venerable Extended Keyboard II and those crappy one-button mice.

To get a more detailed overview, and for the history, read the <A HREF="">Wikipedia page on ADB</A>.

\section adbcables Physical

ADB devices are connected by a 4-pin mini-DIN connector. It's hard to find old ADB cables, but fortunately an s-video cable is the exact same thing (and much more plentiful). The layout of the pins, looking from the front, is:
...
All requests are made by the host and are in bytes. Data is sent MSB first, in order of lowest byte to highest byte. Every bit is a combination of a low pulse and a high pulse for 100us, where the width of each determines which it is.

- \c 0 is 65us low, 35us high.
- \c 1 is 35us low, 65us high.
- \c Reset is a low signal for at least 1s.
...
A request byte sent from the host is constructed in this way. The address that is used can be any four bit value, but after reset keyboards will default to address \c 0x2 and mice to \c 0x3.
...
-# Host signals reset.
-# Host sends talk commands to addresses \c 0x2 and \c 0x3 for register \c 0x3 (device ID).
-# Device responds and moves to higher address (chosen randomly).
-# Host tells device to move from new address to an address it chooses.
...
Devices ask for attention with a service request (Srq) signal. This is a low signal for 300us. An Srq can only be sent by a device during the stop bit cell time if the request is not for it (to a different address).

The default active device is \c 0x3. The host should continuously poll the last active device (that asserted Srq). The device will only respond if it has data to send.

\section adbimpl Implementation

Since ADBUSB only supports one physical connection to one keyboard it takes a few shortcuts with the protocol to simplify the implementation. For starters, the flow to initialize devices to unique addresses is not taken. The full flow, along with any assumptions, is documented below:

-# Host signals reset for 1s.
-# Host delays for 4ms.
-# Host begins an infinite loop of this sequence:
   -# Attention signal (low for 800us).
   -# Sync signal (high for 70us).
   -# Command packet
      - 8 bits (100us each)
